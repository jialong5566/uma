...
import express from '@umajs/bundler-utils/compiled/express';
import cors from 'cors';
import {createWebSocketServer} from "./ws";
import {extname, join} from "path";
import {existsSync} from "fs";
import {logger, getDevBanner} from "@umajs/utils";
import {MESSAGE_TYPE} from "../constants";
import {createHttpsServer, createProxy} from "@umajs/bundler-utils";
...
export async function createServer(opts: IOpts): Promise<any>{
  ...
  // compression
  app.use(require('@umajs/bundler-webpack/compiled/compression')());
  ...
  const webpackDevMiddleware = require('@umajs/bundler-webpack/compiled/webpack-dev-middleware');
  ...
  // history fallback
  app.use(
      require('@umajs/bundler-webpack/compiled/connect-history-api-fallback')({
        index: '/',
      }),
  );

  let server: http.Server | Awaited<ReturnType<typeof createHttpsServer>> = http.createServer(app);
  if (!server) {
    return null;
  }
